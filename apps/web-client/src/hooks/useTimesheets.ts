...
export function useCreateTimesheetBatch() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: (input: { entry_date: string; client_id?: string; project_id?: string; inspection_id?: string; entries: Array<{ work_type: string; hours: number; notes?: string; pre_inspection?: boolean }> }) =>
