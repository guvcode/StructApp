...
export default function MobileShell() {
  ...
  return (
    <div className="min-h-screen bg-paper flex flex-col">
      <header className="px-4 py-2 border-b border-line flex items-center justify-between">
        <div className="flex items-center gap-2">
          <img src="https://www.nerizon.ca/img/new-logo.jpg" alt="Nerizon" className="h-8 w-auto" />
          <span className="text-sm font-bold text-ink">StructApp</span>
        </div>
        <TenantContextBadge />
      </header>
      <main className="flex-1 overflow-y-auto p-4 pb-24">
        {!isOnline && (
          <div className="bg-amber text-ink text-center text-xs py-1 rounded mb-3">
            {isReconnecting
        ...
        {isReconnecting && (
          <div className="bg-blue-tag text-white text-center text-xs py-1 rounded mb-3">
            Syncing {pendingSyncCount} pending change{pendingSyncCount === 1 ? '' : 's'}...
          </div>
        )}
        <InstallPrompt />
