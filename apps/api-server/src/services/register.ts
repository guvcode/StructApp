...
export async function listStructures(siteId?: string, clientId?: string): Promise<Array<Record<string, unknown>>> {
  let query = 'SELECT structure_id, site_id, client_id, name, type, identifier, created_at, updated_at FROM structures';
  const params: unknown[] = [];
  const conditions: string[] = [];
...
export async function searchStructures(queryStr: string, clientId?: string): Promise<Array<Record<string, unknown>>> {
  ...
  let sql = "SELECT structure_id, site_id, client_id, name, type, identifier, created_at, updated_at FROM structures WHERE (name ILIKE $1 OR identifier ILIKE $1)";
...
export async function getStructureById(structureId: string): Promise<Record<string, unknown> | null> {
  const result = await pool.query(
    'SELECT structure_id, site_id, client_id, name, type, identifier, created_at, updated_at FROM structures WHERE structure_id = $1',
    [structureId],
  );
  return result.rows[0] || null;
}
...
export async function createStructure(input: {
  ...
  const result = await pool.query(
    'INSERT INTO structures (site_id, name, type, identifier) VALUES ($1, $2, $3, $4) RETURNING structure_id, site_id, client_id, name, type, identifier, created_at, updated_at',
    [input.site_id, input.name, input.type, input.identifier],
  );
  return result.rows[0];
}
...
export async function updateStructure(structureId: string, fields: Record<string, unknown>): Promise<Record<string, unknown> | null> {
  ...
  const result = await pool.query(
    `UPDATE structures SET ${setClauses.join(', ')} WHERE structure_id = $${idx}
     RETURNING structure_id, site_id, client_id, name, type, identifier, created_at, updated_at`,
    params,
  );
  return result.rows[0] || null;
}